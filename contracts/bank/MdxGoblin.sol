...
contract MdxGoblin is Ownable, ReentrancyGuard, Goblin {
    ...
    address public operator;   // bank 合约地址
        ...
        nonReentrant
    {
        ...
        if (address(staking) != address(0)) {
            ...
            lpBalance = _removePosition(id, user);
        }
        ...
        // 可优化?? lpToken.balanceOf(address(this) 为零时，可以不 transfer
        if (lpBalance > 0) {
            lpToken.transfer(strategy, lpBalance);
        }
        ...
        require(lpAfter >= lpBefore, "lpAfter < lpBefore");
        // 3. Add LP tokens back to the farming pool.
        _addPosition(id, user, lpAfter - lpBefore);
        ...
        nonReentrant
    {
        ...
        _removePosition(id, user);
        // ??? 为什么是把全部支持转移到 liqStrategy ??
        // 因为之前所有的 lp 都在mdex pool中挖矿, _removePosition 是把 lp + mdextoken 赎回来
    ...
    function _addPosition(uint256 id, address user, uint256 lpBalance) internal {
        // 如果 没有stake 操作, lpBalance 不能直接去合约地址的 balance 因为所有的 LP 都在这个合约地址上
        // uint256 lpBalance = lpToken.balanceOf(address(this));
        if (lpBalance > 0) {
            // take lpToken to the pool2.
            if (address(staking) != address(0)) {
                // stake 将 LP 转到 pool 中
    ...
    function _removePosition(uint256 id, address user) internal returns (uint256) {
